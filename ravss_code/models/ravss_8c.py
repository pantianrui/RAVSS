...
class SPKTransformerBlock(nn.Module):
    ...
    def forward(self, speaker, v_memory,            
        ...
        return tgt.transpose(0,1) #(BCK,S,N)
...
class Cross_Triple_Computation_Block(nn.Module):
    ...
    def __init__(
        ...
        #self.speaker_global_mdl = speaker_global_mdl
        ...
        if speaker_self_mdl != None:
            self.speaker_self_norm = select_norm(norm, out_channels, 4)
            #self.speaker_global_norm = select_norm(norm, out_channels, 4)
    ...
    def forward(self, i, x, v_memory,mix_num):
        ...
        B, num_spks, N, K, S = x.shape    #num_spks = 5 
        tmp_b = x
        ...
        # intra RNN
        # [BCS, K, N]
        intra = x.permute(0, 3, 2, 1).contiguous().view(B * self.num_spks * S, K, N)
        # [BCS, K, N]
        intra = self.intra_mdl(intra)
        # [BC, S, K, N]
        intra = intra.view(B * self.num_spks, S, K, N)
        # [BC, N, K, S]
        intra = intra.permute(0, 3, 2, 1).contiguous()
        if self.norm is not None:
            intra = self.intra_norm(intra)
        ...
        v_memory = v_memory.permute(0, 2, 3, 1).contiguous().view(B * mix_num * K, S, N) #(BCK,S,N)
        if self.num_spks > mix_num:
            speaker_v,speaker_nv = speaker.split([B*mix_num*K,B*(self.num_spks-mix_num)*K],dim=0) #(:B*num_spks*K,S,N)+(B*num_spks*K:,S,N)
            speaker_v = self.speaker_cross_mdl(speaker_v,v_memory) #(BCK,S,N)
            speaker = torch.cat([speaker_v,speaker_nv],dim=0)  #(BCK,S,N)
        else:
            speaker = self.speaker_cross_mdl(speaker,v_memory)

        speaker = speaker.transpose(1,2) #(BCK,N,S)
        if self.norm is not None:
        ...
        inter = self.inter_mdl(inter)
        # [BC, K, S, N]
        inter = inter.view(B * self.num_spks, K, S, N)
        # [BC, N, K, S]
        inter = inter.permute(0, 3, 1, 2).contiguous()
        if self.norm is not None:
            inter = self.inter_norm(inter)
        inter = inter.view(B,self.num_spks,N,K,S)

        #[BKS, num_spks, N]
        # speaker_global = inter.permute(0,3,4,1,2).contiguous().view(B * K * S, num_spks, N)
        # v_memory = v_memory.view(B,num_spks,K,S,N)
        # v_memory = v_memory.permute(0,2,3,1,4).view(B*K*S,num_spks,N)
        # speaker_global = self.speaker_global_mdl(speaker_global,v_memory) #[BKS,num_spks,N]
        # speaker_global = speaker_global.transpose(1,2) #[BKS,N,num_spks]
        # if self.norm is not None:
        #     speaker_global = self.speaker_global_norm(speaker_global)
        # speaker_self = speaker_global.transpose(1,2) #[BKS,N,num_spks] -> [BKS,num_spks,N]

        speaker_self = inter.permute(0,3,4,1,2).view(B*K*S,self.num_spks,N) #[BKS,num_spks,N]
        speaker_self = self.speaker_self_mdl(speaker_self)
        speaker_self = speaker_self.view(B,K,S,self.num_spks,N)
        speaker_self = speaker_self.permute(0,3,4,1,2).contiguous().view(B*self.num_spks,N,K,S) #(B * num_spks,N,K,S)
        if self.norm is not None:
            speaker_self = self.speaker_self_norm(speaker_self)
        out = speaker_self.view(B,self.num_spks,N,K,S)
...
class Cross_Sepformer(nn.Module):
    def __init__(self,
        ...
        self.Separator = Cross_Dual_Path_Model(num_spks=num_spks,
                                               ...
                                               speaker_global_model=SpeakerSeparator_Global,
...
def Cross_Sepformer_warpper(kernel_size=16, N_encoder_out=256, num_spks=1):
    ...
    SpeakerSeparator_Cross = SPKTransformerBlock(num_layers=1,
                                        ...
                                        use_positional_encoding=False,
                                        norm_before=True)   
    SpeakerSeparator_Global = SPKTransformerBlock(num_layers=1,
                                        d_model=N_encoder_out,
                                        nhead=8,
                                        d_ffn=1024,
                                        dropout=0,
