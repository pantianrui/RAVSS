...
class SPKTransformerBlock(nn.Module):
    def __init__(
        ...
        # self.linear_w = nn.Linear(d_model,d_model)
    ...
    def forward(self, speaker, v_memory,            
        ...
        # B,C,N,K,S = speaker.shape
        # global_pool = speaker[:,:,:,-1,:] #(B,C,N,S,1)
        # global_pool = torch.mean(global_pool,dim=-1) #(B,C,N)
        # global_pool = global_pool.permute(1,0,2).contiguous() #(C,B,N)
        # speaker_global = self.forward_attention(global_pool,v_memory) #(C,B,N)
        # speaker_global = speaker_global.transpose(0,1) #(B,C,N)
        # out = self.sigmoid(self.linear_s(speaker_global)).unsqueeze(3).unsqueeze(4) * speaker + self.linear_w(speaker_global).unsqueeze(3).unsqueeze(4) * speaker
...
class Query_Decoder(nn.Module):
    def __init__(
        ...
        self.query_embed = nn.Embedding(num_spks+1,d_model)
    ...
    def forward(self,x,memory,v_memory):
        B,_,_ = memory.shape
        ...
        tgt_mask = nn.Transformer.generate_square_subsequent_mask(self.num_spks+1)
        ...
        out = self.linear_a(out_query[:,:self.num_spks,:]).unsqueeze(3).unsqueeze(4) * x.unsqueeze(1) + self.linear_b(out_query[:,:self.num_spks,:]).unsqueeze(3).unsqueeze(4)
        
        out_attractor = self.sigmoid(self.linear_down(out_query).squeeze(-1))
        #(B,2,N,K,S)
        return out, out_attractor

class SPGM(nn.Module):
    def __init__(self,):
        super(SPGM,self).__init__()
        self.linear_s = nn.Linear(256,256)
        self.linear_w = nn.Linear(256,256)
        self.sigmoid = nn.Sigmoid()

    def forward(self,x):
        global_pool = x[:,-1:,:,:] #(B,1,S,N)
        global_pool = torch.mean(global_pool,dim=-2) 
        global_pool = global_pool.unsqueeze(-2) #(B,1,1,N)
        inter = self.sigmoid(self.linear_s(global_pool)) * x + self.linear_w(global_pool) * x
...
class Cross_Dual_Computation_Block(nn.Module):
    ...
    def __init__(
        ...
        #     out_channels, out_channels*2, 1, batch_first=True, bidirectional=True
        # )
        # self.intra_linear = nn.Linear(out_channels*4, out_channels)
        # self.inter_rnn = nn.LSTM(
        #     out_channels, out_channels*2, 1, batch_first=True, bidirectional=True
        # )
        # self.inter_linear = nn.Linear(out_channels*4, out_channels)
        ###########################################################################
    ...
    def forward(self, x):
        ...
        ###################################################################
        # tmp = intra
        # intra, _ = self.intra_rnn(intra)  # [BS, K, N]
        # intra = self.intra_linear(intra)  # [BS, K, N]
        # intra = intra + tmp
        ####################################################################
        ...
        ###################################################################
        # tmp = inter
        # inter, _ = self.inter_rnn(inter)  # [BK, S, N]
        # inter = self.inter_linear(inter)  # [BK, S, N]
        # inter = inter + tmp
        ####################################################################
...
class Cross_Triple_Computation_Block(nn.Module):
    ...
    def forward(self, i, x, v_memory):
        ...
        """
        if i==0:
            x = x.unsqueeze(1).repeat(1,self.num_spks,1,1,1) #(B,C,N,K,S)
        ...
        ###################################################################
        # tmp = intra
        # intra, _ = self.intra_rnn(intra)  # [BCS, K, N]
        # intra = self.intra_linear(intra)  # [BCS, K, N]
        # intra = intra + tmp
        ####################################################################
        intra = self.intra_mdl(intra)
        # [BC, S, K, N]
        ...
        speaker = self.speaker_mdl(speaker,v_memory) #(BCK,N,S)
        if self.norm is not None:
            speaker = self.speaker_norm(speaker)

        # inter RNN
        # [BCK, S, N]
        #inter = intra.permute(0, 2, 3, 1).contiguous().view(B * num_spks * K, S, N)
        inter = speaker.transpose(1,2) #(BCK,S,N)
        ###################################################################
        # tmp = inter
        # inter, _ = self.inter_rnn(inter)  # [BCK, S, N]
        # inter = self.inter_linear(inter)  # [BCK, S, N]
        # inter = inter + tmp
        ####################################################################
        inter = self.inter_mdl(inter)
        # [BC, K, S, N]
...
class Cross_Dual_Path_Model(nn.Module):
    ...
    def __init__(
        ...
        #self.query_decoder = Query_Decoder(num_spks,2,out_channels,8,1024)
        ...
        # self.linear_down = nn.Linear(in_channels,out_channels)
        # self.linear_up = MLP(out_channels,out_channels,in_channels,2)
        # self.norm_linear = nn.LayerNorm(out_channels)
    ...
    def forward(self, x, video):
        ...
        # x = self.conv2d(x)
        # B,_,K,S = x.shape
        # x = x.reshape(B,-1,self.num_spks,K,S)
        # x = x.transpose(1,2) #(B,C,N,K,S)

        # ############################ 把这个映射换成用视觉特征做query的后处理 #############################
        # memory = self._over_add(x, gap) #(B,N,L)
        # x,out_attractor = self.query_decoder(x, memory,v)
        # ############################################################################################
        # B, _, N, K, S = x.shape

        #############################################################################################
        for i in range(self.num_layers):
            x = self.triple_mdl[i](i,x,v)
        x = self.prelu_2(x)
        #############################################################################################
        ...
        # B, _, N, K, S = x.shape
        # x = x.view(B * self.num_spks, -1, K*S)
        # x = x.transpose(1,2).contiguous() #(B*num_spks,K*S,N)
        # x = self.norm_linear(x)
        # x = self.linear_up(x)
        # x = x.transpose(1,2).contiguous() #(B*num_spks,N,K*S)
        # x = x.view(B*self.num_spks,-1,K,S)
        # x = self._over_add(x, gap) #(B*num_spks,N,L)
        # x = x.view(B, self.num_spks, N, -1)
        # x = x.transpose(0,1) #(num_spks,B,N,L)
...
class Cross_Sepformer(nn.Module):
    def __init__(self,
        ...
        self.Separator = Cross_Dual_Path_Model(num_spks=num_spks,
                                               ...
                                               speaker_model=SpeakerSeparator,
                                               norm='ln',
                                               skip_around_intra=True)
    ...
    def forward(self, mix, video):
        ...
        est_mask,out_attractor = self.Separator(mix_w,video)
        mix_w = torch.stack([mix_w] * self.num_spks) #(1,2,256,1999)
...
def Cross_Sepformer_warpper(kernel_size=16, N_encoder_out=256, num_spks=2):
    ...
    IntraSeparator_1 = SBTransformerBlock(num_layers=4,
                                        ...
                                        use_positional_encoding=True,
                                        norm_before=True)
    InterSeparator_1 = CrossTransformerBlock(num_layers=4,
                                           d_model=N_encoder_out,
                                           nhead=8,
                                           d_ffn=1024,
                                           dropout=0,
    ...
    InterSeparator = CrossTransformerBlock(num_layers=8,
                                           ...
                                           use_positional_encoding=False,
                                           norm_before=True)
    SpeakerSeparator = SPKTransformerBlock(num_layers=1,
                                        d_model=N_encoder_out,
                                        nhead=8,
                                        d_ffn=1024,
                                        dropout=0,
