...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_pre(self, tgt, memory, v_memory, 
                    tgt_mask: Optional[Tensor] = None,
                    memory_mask: Optional[Tensor] = None,
                    tgt_key_padding_mask: Optional[Tensor] = None,
                    memory_key_padding_mask: Optional[Tensor] = None,
        ...
        #########################################################################
        _,B,_ = tgt.shape
        tgt2 = self.norm4(tgt) #(2,B,N) # v_memory (25,2*B,N)
        tgt2 = tgt2.transpose(0,1)  #(B,2,N)
        tgt2 = tgt2.reshape(1,B*2,-1) #(1,B*2,N)
        tgt2 = self.multihead_attn_1(query=self.with_pos_embed(tgt2, query_pos),
                                   ...
                                   key_padding_mask=memory_key_padding_mask)[0]
        tgt2 = tgt2.reshape(B,2,-1)
        tgt2 = tgt2.transpose(0,1) #(2,B,N)
        tgt = tgt + self.dropout4(tgt2)
        #########################################################################
        tgt2 = self.norm2(tgt)
        tgt2 = self.multihead_attn_2(query=self.with_pos_embed(tgt2, query_pos),
                                   key=self.with_pos_embed(memory, pos),
                                   value=memory, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)[0]
        tgt = tgt + self.dropout2(tgt2)
    ...
    def forward(self, tgt, memory, v_memory,
                tgt_mask: Optional[Tensor] = None,
                memory_mask: Optional[Tensor] = None,
                tgt_key_padding_mask: Optional[Tensor] = None,
                memory_key_padding_mask: Optional[Tensor] = None,
        ...
        if self.normalize_before:
            return self.forward_pre(tgt, memory, v_memory, tgt_mask, memory_mask,
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory, v_memory,
        ...
        for layer in self.layers:
            output = layer(output, memory, v_memory, tgt_mask=tgt_mask,
...
class Query_Decoder(nn.Module):
    ...
    def forward(self,x,memory,video):
        B,_,_ = memory.shape
        memory = memory.permute(2,0,1).contiguous() #(1999,B,256)
        v = self.visual_conv(video)
        v = v.permute(2,0,1).contiguous() #(25,2*B,256) #ABCD
        ######################################################################################
        # query = self.query_embed.weight.unsqueeze(0).repeat(B, 1, 1) #(2,2,256)
        # query = query.reshape(1,B*2,-1) #(1,B*2,N) #ZXZX
        # memory = memory.unsqueeze(2).repeat(1,1,2,1) #(1999,B,2,256)
        # memory = memory.reshape(-1,B*2,256) #(1999,B*2,256) #(AB)(AB)(CD)(CD)
        #######################################################################################
        query = self.query_embed.weight.unsqueeze(1).repeat(1,B,1) #(2,B,N)

        out_query = self.decoder(query,memory,v) #(2,B,N)
        #######################################################################################
        #out_query = out_query.reshape(B,2,-1) #(B,2,N) x.shape (B,N,K,S)
        out_query = out_query.transpose(0,1) #(B,2,N)
        #######################################################################################

        out = self.linear_a(out_query).unsqueeze(3).unsqueeze(4) * x.unsqueeze(1) + self.linear_b(out_query).unsqueeze(3).unsqueeze(4)
...
class Cross_Triple_Computation_Block(nn.Module):
    ...
    def forward(self, x):
        ...
        speaker = out.view(B,num_spks,N,K,S)
        #[BKS, num_spks, N]
        speaker = speaker.permute(0,3,4,1,2).contiguous().view(B * K * S, num_spks, N)
        speaker = self.speaker_mdl(speaker)
        speaker = speaker.view(B,K,S,num_spks,N)
        speaker = speaker.permute(0,3,4,1,2).contiguous().view(B*num_spks,N,K,S) #(B * num_spks,N,K,S)
        if self.norm is not None:
            speaker = self.speaker_norm(speaker)
        out = out + speaker
        out = out.view(B,num_spks,N,K,S)
...
class Cross_Dual_Path_Model(nn.Module):
    ...
    def __init__(
        ...
        self.query_decoder = Query_Decoder(4,256,8,1024)
    ...
    def forward(self, x, video):
        ...
        #video.shape (1,25,512)
        #print("video.shape",video.shape) video.shape torch.Size([2, 72, 1024])
        ...
        x = self.query_decoder(x, memory,video)
        ############################################################################################
        B, _, _, K, S = x.shape
...
def Cross_Sepformer_warpper(kernel_size=16, N_encoder_out=256, num_spks=2):
    ...
    IntraSeparator = SBTransformerBlock(num_layers=1,
                                        d_model=N_encoder_out,
                                        nhead=8,
                                        d_ffn=1024,
                                        dropout=0,
    ...
    InterSeparator = CrossTransformerBlock(num_layers=2,
                                           ...
                                           use_positional_encoding=False,
                                           norm_before=True)
    SpeakerSeparator = SBTransformerBlock(num_layers=2,
                                        d_model=N_encoder_out,
                                        nhead=8,
                                        d_ffn=1024,
                                        dropout=0,
...
if __name__ == '__main__':
    ...
    print(model(torch.randn(2,16000,8), torch.randn(4, 25, 512)).shape)